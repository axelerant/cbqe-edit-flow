...
abstract class Aihrus_Settings {
	...
	public static function sections() {
		static::$sections['reset'] = esc_html__( 'Reset', 'cbqe-edit-flow' );
		static::$sections['about'] = esc_html__( 'About', 'cbqe-edit-flow' );
	}
	...
	public static function settings() {
		...
		if ( ! empty( $options ) ) {
			...
			static::$settings['export'] = array(
				'section' => 'reset',
				'title' => esc_html__( 'Export Settings', 'cbqe-edit-flow' ),
				'type' => 'readonly',
				'desc' => esc_html__( 'These are your current settings in a serialized format. Copy the contents to make a backup of your settings.', 'cbqe-edit-flow' ),
		...
		static::$settings['import'] = array(
			'section' => 'reset',
			'title' => esc_html__( 'Import Settings', 'cbqe-edit-flow' ),
			'type' => 'textarea',
			'desc' => esc_html__( 'Paste new serialized settings here to overwrite your current configuration.', 'cbqe-edit-flow' ),
			'widget' => 0,
		);

		$desc = esc_html__( 'Delete all %s data and options from database on plugin deletion', 'cbqe-edit-flow' );

		static::$settings['delete_data'] = array(
			'section' => 'reset',
			'title' => esc_html__( 'Remove Plugin Data on Deletion?', 'cbqe-edit-flow' ),
			'type' => 'checkbox',
			'class' => 'warning',
		...
		static::$settings['reset_defaults'] = array(
			'section' => 'reset',
			'title' => esc_html__( 'Reset to Defaults?', 'cbqe-edit-flow' ),
			'type' => 'checkbox',
			'class' => 'warning',
			'desc' => esc_html__( 'Check this box to reset options to their defaults', 'cbqe-edit-flow' ),
		);
	}
...
';
	...
	public static function display_page( $disable_donate = false ) {
		...
		echo '
			<p class="submit"><input name="Submit" type="submit" class="button-primary" value="' . esc_html__( 'Save Changes', 'cbqe-edit-flow' ) . '" /></p>
			</form>
		</div>
		';
		...
		if ( ! $disable_donate ) {
			echo '<p>' .
				sprintf(
				__( 'If you like this plugin, please <a href="%1$s" title="Donate for Good Karma"><img src="%2$s" border="0" alt="Donate for Good Karma" /></a> or <a href="%3$s" title="purchase premium WordPress plugins from Aihrus ">purchase the Premium version</a> to help fund further development and <a href="%4$s" title="Support forums">support</a>.', 'cbqe-edit-flow' ),
		...
		echo '<p class="copyright">' .
			sprintf(
			__( 'Copyright &copy;%1$s <a href="%2$s">Aihrus</a>.', 'cbqe-edit-flow' ),
			date( 'Y' ),
	...
	public static function validators( $validate, $id, &$input, $default, &$errors ) {
		switch ( $validate ) {
			...
			case 'required':
				if ( empty( $input[ $id ] ) )
					$errors[ $id ] = esc_html__( 'Required', 'cbqe-edit-flow' );
				break;
